...
from Algorithms.density_greedy_algo import dgalgo


def run(self,bvt,dt,vut,ont, max_weight):
    array = import_csv(self.fname[:len(self.fname)-4])
    duration, best_value, volume_taken, objs, vols,values, nb_exem = dgalgo(max_weight, array)
...
def densitygreedy(self):
    ...
    welcome = QLabel('Density Greedy Algorithm')
