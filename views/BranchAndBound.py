...
def run(self,bvt,dt,vut,ont, weight):
    array = import_csv(self.fname[:len(self.fname)-4])
    duration, best_value, volume_taken, objs, vols,values, nb_exem = BranchBound(weight, array)
...
def branchTab(self):
    ...
    p2, pt1 = parameter_input(self,name='pdddd     ')
    p3, pt2 = parameter_input(self,name='pdgfgfgd  ')
    p4,pt3 = parameter_input(self,name='pddfffffdd')
    ...
    calcualte.clicked.connect(lambda: run(
        self,bvt,dt,vut,ont ,float(pmw.text())))
